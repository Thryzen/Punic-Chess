...
function captureScore(state, action) {
  const captured = (action.captures ?? []).map((id) => getPieceById(state, id)).filter(Boolean);
  const capturedValue = captured.reduce((total, piece) => total + pieceValue(piece), 0);
  const moving = getPieceById(state, action.pieceId);
  const clashCost = action.kind === "guard-clash" ? pieceValue(moving) * 0.8 : 0;
  return capturedValue * 12 - clashCost;
}
...
function replyRiskScore(state, side) {
  ...
  for (const action of getAllActions(opponentState, opponent)) {
    ...
    let score = captureScore(opponentState, action);
...
function scoreAction(state, action, side, effects) {
  ...
  if (next.winner === side) return 100000 + captureScore(state, action);

  let score = 0;
  score += captureScore(state, action);
